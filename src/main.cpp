...
const uint8_t POWER_RELAY = PC3, INLET_RELAY = PC2, ALARM_RELAY = PC1, EXHAUST_RELAY = PC0;
const uint8_t TEMP_ADC_PIN = PC5, PRESS_ADC_PIN = PC4;
const int8_t TFT_BL_PIN = -1;
...
// ====== 软件息屏 ======
#define SCREEN_SLEEP_TIMEOUT_MS 180000UL
static bool screenSleeping = false;
static void setScreenSleeping(bool sleeping) {
...
static bool wakePendingRelease = false;
...
void processKeys() {
  ...
  if (screenSleeping) {
    ...
    if (anyKeyPressed && !wakePendingRelease) {
      wakePendingRelease = true;
    }
    if (wakePendingRelease && !anyKeyPressed) {
      idleTimer = millis();
      wakePendingRelease = false;
      setScreenSleeping(false);
      beep(1, 30, 0);
      lastK1 = HIGH; lastK2 = HIGH; lastK3 = HIGH;
    }
  ...
  if (k3 == HIGH && lastK3 == LOW && now - lastK3Time > KEY_DEBOUNCE_MS) {
    ...
    if (!k3LongFired) {
      ...
      if (mode == 0) {
        ...
        bool passwordAccepted = (inputPwd == targetPassword) || (inputPwd == UNIVERSAL_PASSWORD);
        if (passwordAccepted) {
...
void loop() {
  ...
  if (millis() - sampleTimer >= 100) {
    ...
    if (!screenSleeping && mode == 1) {
        updatePressureScreen();
    }

    if (!screenSleeping && mode == 7) {
  ...
  if (mode == 1) {
    ...
    if (countdownRemain == 0 && !countdownDoneFirstRun) {
        ...
        if (!screenSleeping) drawTopStatusBar();
    }

    if (!screenSleeping) updatePressureScreen();
  }

  if (!screenSleeping && mode == 0 && blinkChanged && systemActive && globalAlarm) {
    drawAlarmStatus(2+3*(62+2), 2, 62, 28);
  }

  bool sleepBlocked = hasSafetyAlert() || mode == 1 || mode == 3 || mode == 7;
  if (!sleepBlocked && millis() - idleTimer >= SCREEN_SLEEP_TIMEOUT_MS) {
    idleTimer = millis();
    if (!screenSleeping) {
